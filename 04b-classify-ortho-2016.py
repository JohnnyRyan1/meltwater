...
# Define files
files = sorted(glob.glob(path + 'drone/20160705/Orthomosaics/*.tif'))

# Read
df = pd.read_csv(path + 'ndwi-thresholds-2016.csv')
...
for file in files:
    ...
    if os.path.exists(path + '/drone/20150721/Classified/' + infileshortname + '.tif'):
        ...
        if np.max(image[0,:,:]).values == 0:
            ...
            classified.rio.to_raster(path + '/drone/20160705/Classified/' + infileshortname + '.tif')
            ...
            # Save to file
            gdf.to_file(path + 'drone/20160705/Polys/' + infileshortname + '.shp')
